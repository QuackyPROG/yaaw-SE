...
from yaaw.planning_quality import acceptance_issues
...
from yaaw.retrieval import plan_retrieval
...
def _ticket(data: dict) -> Ticket:
    return Ticket(id=data["id"], kind=TicketKind(data.get("kind","DELIVERY")), status=TicketState(data.get("status","DRAFT")), level=int(data.get("level",1)), owner=data.get("owner","owner"), blocked_by=tuple(data.get("blocked_by",[])), qa_required=bool(data.get("qa_required",False)), acceptance=tuple(data.get("acceptance",["observable outcome"])))
...
def evaluate(scenario: dict) -> tuple[bool, object, object]:
    ...
    if kind=="route":
    ...
    elif kind=="planning_lint": actual=acceptance_issues(inp.get("acceptance",[]))
    elif kind=="retrieval_plan": actual=[item.hook for item in plan_retrieval(inp["path"])]
    elif kind=="graph":
        ...
        count=int(inp.get("count",100)); tickets=[Ticket("T000",TicketKind.DELIVERY,TicketState.DONE,3,"core"),Ticket("T001",TicketKind.DELIVERY,TicketState.READY,3,"core",("T000",))]
        for index in range(2,count): tickets.append(Ticket(f"T{index:03d}",TicketKind.DELIVERY,TicketState.DRAFT,3,"core",(f"T{index-1:03d}",)))
        graph=TicketGraph(tickets); d=graph.diagnostics(); actual={"tickets":len(graph.tickets),"cycles":len(d.cycles),"frontier":[t.id for t in graph.ready_frontier()]}
        ...
        recorded=inp.get("recorded_fingerprints",{"spec":"a"}); current=inp.get("current_fingerprints",recorded); record=EvidenceRecord.create(verification_id=inp.get("verification_id","unit"),command="test-command",exit_code=int(inp.get("exit_code",0)),environment="CI",commit=inp.get("recorded_commit","abc"),source_fingerprints=recorded)
    ...
    elif kind=="failure_signature_replan":
        from yaaw.budgets import Budget
        from yaaw.controller import AdmissionError, Controller
        from yaaw.leases import LeaseStore
        import tempfile
        with tempfile.TemporaryDirectory() as td:
            controller=Controller(TicketGraph([]),Budget({"max_same_failure_signature":int(inp.get("limit",2))}),LeaseStore(Path(td)/"leases"))
            allowed=True
            try:
                for _ in range(int(inp.get("count",3))): controller.register_failure(inp.get("signature","same"))
