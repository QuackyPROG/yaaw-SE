...
def reconcile_observed(observed: dict[str, Any]) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    for ticket_id, ticket in current.get("tickets", {}).items():
        ...
        verification_present = ticket.get("verification_present", False)
        legacy_fresh = ticket.get("fresh_review")
        review_present = ticket.get("review_present", bool(legacy_fresh))
        review_source_current = ticket.get("review_source_current", source_current)
        review_repository_current = ticket.get("review_repository_current", bool(legacy_fresh))
        verification_repository_current = ticket.get("verification_repository_current", verification_present)
        ...
        if state == "PASS":
            if not source_current or not review_source_current:
                ...
                changes.append(_change(ticket_id, "PASS", "REPLAN_REQUIRED", "TICKET_SOURCE_STALE"))
                continue
            if not review_present:
                ticket["state"] = "REVIEW_REQUIRED"
                changes.append(_change(ticket_id, "PASS", "REVIEW_REQUIRED", "REVIEW_MISSING"))
                continue
            if not identity_verifiable:
                ticket["state"] = "REVIEW_REQUIRED"
                changes.append(_change(ticket_id, "PASS", "REVIEW_REQUIRED", "LEGACY_IDENTITY_UNVERIFIABLE"))
                continue
            if not review_repository_current:
                ticket["state"] = "REVIEW_REQUIRED"
                changes.append(_change(ticket_id, "PASS", "REVIEW_REQUIRED", "REVIEW_REPOSITORY_STALE"))
                continue
            if not verification_present:
                ticket["state"] = "REVIEW_REQUIRED"
                changes.append(_change(ticket_id, "PASS", "REVIEW_REQUIRED", "VERIFICATION_MISSING"))
                continue
            if not verification_repository_current:
                ticket["state"] = "REVIEW_REQUIRED"
                changes.append(_change(ticket_id, "PASS", "REVIEW_REQUIRED", "VERIFICATION_REPOSITORY_STALE"))
                continue

        if state == "IN_PROGRESS" and implementation and verification_present and not review_repository_current:
            ticket["state"] = "REVIEW_REQUIRED"
