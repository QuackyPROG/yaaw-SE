...
def main() -> None:
    ...
    require(config.get("agents", {}).get("max_concurrent_threads_per_session") <= 3, "spawned-child cap must remain <= 3")
    ...
    for phrase in ("Only the root Orchestrator may spawn", "one mutating agent", "STOP_AND_REPLAN", ".agents/artifacts.json"):
    ...
    for path in sorted((ROOT / ".codex/agents").glob("*.toml")):
        ...
        require("model" not in data and "model_reasoning_effort" not in data, f"{path.name} hard-codes model policy; keep runtime profiles separate")
    ...
    print("OK: Codex adapter is root-only, bounded, model-neutral, artifact-aware, and aligned with registered roles")
