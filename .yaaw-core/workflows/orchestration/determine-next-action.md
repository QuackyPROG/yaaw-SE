...
3. Resolve its repository requirement from `registries/execution-policy.json`.
...
5. Resolve exact read/write/forbidden-write artifact classes from `registries/role-io.json` and exact current artifact references.
6. Select only YAAW expertise relevant to the selected workflow/artifact. Host expertise is admitted only through `rules/research-admission.md`.
...
Before persisting a handoff, verify that its write set is a subset of the selected role's canonical artifact writes and contains no installer-managed artifact or package-managed framework path. A framework defect is a stop condition, never a target workflow.
