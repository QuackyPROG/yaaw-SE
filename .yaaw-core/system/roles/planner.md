...
## Required behavior
- Resolve workspace/repository context before repository discovery; repository commands are root-anchored through `.yaaw-core/system/core/execution-context.md`.
...
- Apply `.yaaw-core/system/rules/assumption-challenge.md` to engineering interpretation, architecture decisions, and current-frontier questions.
...
- Apply `.yaaw-core/system/rules/research-admission.md` before web/vendor research or host expertise. An installed host skill is never architectural evidence by itself.
- Persist material blocking research as `RSH-*`; research findings do not automatically become `ENG-*` decisions.
- Apply `.yaaw-core/system/rules/changeability.md` when shaping engineering decisions, specifications, and ticket boundaries.
